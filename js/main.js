...
function openOverlay(){overlay.classList.add("is-open");overlay.setAttribute("aria-hidden","false");screenRoot.classList.add("screen-overlay-open")}
function closeOverlay(ev){if(ev)ev.stopPropagation();overlay.classList.remove("is-open");overlay.setAttribute("aria-hidden","true");screenRoot.classList.remove("screen-overlay-open")}
...
function createScene(slot){
  ...
  const key = new THREE.DirectionalLight(0xffffff,2.25); key.position.set(2.0,1.4,3.0); scene.add(key);
  const fill = new THREE.DirectionalLight(0xffffff,0.85); fill.position.set(-1.6,-0.7,2.0); scene.add(fill);
  const rim = new THREE.DirectionalLight(0xffffff,1.0); rim.position.set(0.1,0.0,-2.4); scene.add(rim);
  ...
  composer.addPass(new UnrealBloomPass(new THREE.Vector2(256,256), mode==="overlay"?0.18:0.10, 0.40, 0.88));

  const state = {slot,mode,renderer,composer,camera,pivot,modelWrap,root:null,modelRadius:1};
    ...
    root.traverse((obj)=>{
      ...
      for(const mat of materials){
        ...
        if("emissive" in mat){
          ...
          mat.emissiveIntensity = mode==="overlay" ? 0.18 : 0.10;
        }
  ...
  function resize(){
    ...
    const fitFactor = mode==="overlay" ? 0.69 : 0.36;
    const dist = (radius / Math.sin(fov/2)) * fitFactor;
    camera.position.set(mode==="overlay" ? 0.05 : 0.004, mode==="overlay" ? 0.012 : 0.001, dist);
    camera.lookAt(0,0,0);
  ...
  new GLTFLoader().load(modelPath,(gltf)=>{
    ...
    const targetRadius = mode==="overlay" ? 3.65 : 0.14;
...
function animate(){
  ...
  for(const s of scenes){
    const drift = s.mode==="overlay" ? 0.008 : 0.0008;
    s.pivot.position.y = Math.sin(t*0.16)*drift;
    s.pivot.position.x = Math.cos(t*0.11)*drift*0.65;
    if(s.root){
      s.root.rotation.y += s.mode==="overlay" ? 0.0010 : 0.00016;
      s.root.rotation.x += s.mode==="overlay" ? 0.00006 : 0.000008;
    }
    s.composer.render();
  }
