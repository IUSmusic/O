...
function createScene(slot) {
  ...
  function resize() {
    ...
    const fitFactor = mode === "overlay" ? 0.685 : 0.63; // smaller bottom object
    const dist = (radius / Math.sin(fov / 2)) * fitFactor;

    camera.position.set(mode === "overlay" ? 0.06 : 0.025, mode === "overlay" ? 0.015 : 0.008, dist);
    camera.lookAt(0, 0, 0);
  ...
  loader.load(modelPath, (gltf) => {
    ...
    const targetRadius = mode === "overlay" ? 3.68 : 0.58;
