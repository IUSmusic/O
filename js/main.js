...
function createScene(slot) {
  ...
  function resize() {
    ...
    const fitFactor = mode === "overlay" ? 0.69 : 0.50;
    const dist = (radius / Math.sin(fov / 2)) * fitFactor;
    camera.position.set(mode === "overlay" ? 0.05 : 0.016, mode === "overlay" ? 0.012 : 0.004, dist);
    camera.lookAt(0, 0, 0);
  ...
  loader.load(modelPath, (gltf) => {
    ...
    const targetRadius = mode === "overlay" ? 3.68 : 0.37;
...
function animate() {
  ...
  for (const s of scenes) {
    const drift = s.mode === "overlay" ? 0.008 : 0.0025;
    s.pivot.position.y = Math.sin(t * 0.16) * drift;
    s.pivot.position.x = Math.cos(t * 0.11) * drift * 0.65;
    s.pivot.rotation.z = Math.sin(t * 0.09) * (s.mode === "overlay" ? 0.006 : 0.0025);

    if (s.root) {
      s.root.rotation.y += s.mode === "overlay" ? 0.0010 : 0.0005;
      s.root.rotation.x += s.mode === "overlay" ? 0.00006 : 0.00002;
    }
    s.composer.render();
  }
