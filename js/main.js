...
function createScene(slot) {
  ...
  function resize() {
    ...
    const fitFactor = mode === "overlay" ? 0.69 : 0.39;
    const dist = (radius / Math.sin(fov / 2)) * fitFactor;
    camera.position.set(mode === "overlay" ? 0.05 : 0.005, mode === "overlay" ? 0.012 : 0.001, dist);
    camera.lookAt(0, 0, 0);
  ...
  loader.load(modelPath, (gltf) => {
    ...
    const targetRadius = mode === "overlay" ? 3.65 : 0.17;
...
function animate() {
  ...
  for (const s of scenes) {
    const drift = s.mode === "overlay" ? 0.008 : 0.001;
    s.pivot.position.y = Math.sin(t * 0.16) * drift;
    s.pivot.position.x = Math.cos(t * 0.11) * drift * 0.65;
    if (s.root) {
      s.root.rotation.y += s.mode === "overlay" ? 0.0010 : 0.00022;
      s.root.rotation.x += s.mode === "overlay" ? 0.00006 : 0.00001;
    }
    s.composer.render();
  }
